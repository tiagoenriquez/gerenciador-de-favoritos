...
class AmeacaFrame:
    def __init__(self, frame: MyFrame, assunto: Assunto) -> None:
        ...
        self._titulo_label = tk.Label(
            self._panel,
            text=f"Tem certeza de que deseja excluir o assunto {assunto.nome}?",
            bg=frame.background,
            fg=frame.foreground,
    ...
    def _desistir(self) -> None:
        AssuntoController.abrir_seletor(self._my_frame)
    

    def _excluir(self) -> None:
        AssuntoController.excluir(self._my_frame, self._assunto.id)